...
import { ref, defineComponent, computed, mergeProps, unref, withCtx, createTextVNode, createVNode, createBlock, openBlock, useSSRContext } from 'vue';
...
import { c as useRoute, b as useRouter, a as useAuth } from './server.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const termId = ref(route.params.id);
    ...
    const openBulkUpdateModal = (courseName, targetWeek, newStatus) => {
            ...
            ssrRenderList(dayEntries, (entry) => {
              ...
              if (!entry.isPast) {
                ...
                ssrRenderList(["Gittim", "Gitmedim", "Tatil / Ders Yok"], (status) => {
                  ...
                  ])}"${ssrIncludeBooleanAttr(unref(term).isReadOnly) ? " disabled" : ""} onselectstart="return false">${ssrInterpolate(status)}</button>`);
                });
                _push(`<!--]--></div>`);
              }
              _push(`</div></div>`);
