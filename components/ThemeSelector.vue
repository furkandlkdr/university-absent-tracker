<template>
	<div class="theme-selector bg-surface-light dark:bg-surface-dark rounded-lg shadow-lg p-4 w-full max-w-xs">
			...
			<div class="mt-3">
				<div class="relative w-full mb-1">
					<input type="range" min="0" max="360" step="1" v-model="hue" @input="updateHue"
						class="w-full h-4 rounded-lg appearance-none cursor-pointer" :style="colorSliderStyle" />
					...
					<div class="flex items-center justify-between mt-2">
						<div class="flex space-x-1">
							<div v-for="chroma in [0.05, 0.1, 0.15, 0.19]" :key="chroma"
								...
								:style="{ backgroundColor: `oklch(68% ${chroma} ${hue})` }"></div>
						</div>
						<div class="text-xs text-gray-600 dark:text-gray-300">{{ hue }}°</div>
					</div>

					<!-- Material You Style Preview -->
					<div
						class="mt-3 p-2 rounded-md flex items-center justify-center space-x-2 border border-gray-200 dark:border-gray-700">
